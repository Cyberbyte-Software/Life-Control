    ...
    <div class="col-md-4" style="float:left;  padding-top:20px;">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h2 class="panel-title"><i class="fa fa-child fa-fw"></i><?php echo nameID($row["pid"])."'s ". carName($row["classname"]); ?></h2>
            </div>
            <div class="panel-body">
				<center><img src="assets/img/cars/<?php echo $row['classname'];?>.jpg" />
                    <?php
                    ...
                    } else echo "<h1 style='color:red'>ERROR NO RESULTS</h1>";
                    echo "</center>";
                    ?>
            </div>
        </div>
	...
	<div class="col-md-8" style="float:right; padding-top:20px;">
		...
		<div class="panel panel-default" style="float:left; width:100%; margin:0 auto;">
			<ul id="myTab" class="nav nav-tabs">		
				<li><a href="#veh_inv" data-toggle="tab"><?php echo $lang['inventory'];?></a></li>
			</ul>
...
<div class="modal fade" id="edit_veh_inv" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><span class="glyphicon glyphicon-pencil"></span><?php echo " ".$lang['edit']." ".$lang['vehicle']." ".$lang['inventory'];?></h4>
            </div>
			<?php 
				$sql = 'SELECT * FROM `vehicles` WHERE `id` ="' . $vehID . '";';
				$result_of_query = $db_connection->query($sql);
            ...
            <form method="post" action="editVeh.php?ID=<?php echo $vehID;?>" role="form">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="row">
                            <textarea class="form-control" rows="10" name="vehInv"><?php echo $row['inventory'];?></textarea>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="del_veh" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><span class="glyphicon glyphicon-pencil"></span><?php echo " ".$lang['DELETE']." ".$lang['vehicle'];?></h4>
            </div>
			<?php 
				$sql = 'SELECT * FROM `vehicles` WHERE `id` ="' . $vehID . '";';
				$result_of_query = $db_connection->query($sql);
...
<div class="modal fade" id="store_veh" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><span class="glyphicon glyphicon-pencil"></span><?php echo " ".$lang['store']." ".$lang['vehicle'];?></h4>
            </div>
			<?php 
				$sql = 'SELECT * FROM `vehicles` WHERE `id` ="' . $vehID . '";';
				$result_of_query = $db_connection->query($sql);
